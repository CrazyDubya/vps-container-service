...
class Database {
    ...
    async getUserByApiKey(apiKey) {
        // Get all active users and check API key hash
        return new Promise((resolve, reject) => {
            this.db.all(
                ...
                async (err, rows) => {
                    ...
                    // Check each user's API key hash
                    for (const user of rows) {
                        try {
                            // If API key is hashed (starts with $2b$), compare with bcrypt
                            if (user.api_key && user.api_key.startsWith('$2b$')) {
                            ...
                            // Legacy support: direct comparison for non-hashed keys
                            else if (user.api_key === apiKey) {
                                resolve(user);
                                return;
                            }
                        } catch (compareError) {
    ...
    async regenerateApiKey(userId) {
        ...
        return new Promise((resolve, reject) => {
            this.db.run(
                ...
                (err) => {
                    if (err) reject(err);
                    else resolve(newApiKey); // Return plain text for display ONCE
                }
            );
        });
    }
