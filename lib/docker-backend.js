...
class DockerBackend extends ContainerInterface {
  ...
  async attachInteractive(id, ws) {
    ...
    // Start the exec with hijack mode for interactive communication
    const stream = await exec.start({
      hijack: true,
      stdin: true
    });
  ...
  async copyTo(id, sourcePath, targetPath) {
    const container = this.docker.getContainer(id);
    const fs = require('fs');
    ...
    const fileName = require('path').basename(targetPath);
    ...
    await container.putArchive(pack, {
      path: require('path').dirname(targetPath) || '/'
    });
  }

  async copyFrom(id, sourcePath, targetPath) {
    const container = this.docker.getContainer(id);
    const fs = require('fs');
    ...
    return new Promise((resolve, reject) => {
      ...
      extract.on('finish', () => resolve(targetPath));
