...
class LXDBackend extends ContainerInterface {
  ...
  async _performAsyncCreation(containerName, config) {
    try {
      ...
      await new Promise((resolve, reject) => {
        const proc = spawn('lxc', ['launch', image, containerName, '--config', `limits.memory=${config.memory || 512}MB`]);
        let stderr = '';
      ...
      this.creationStatus.set(containerName, {
        status: 'creating',
        progress: 80,
        message: 'Configuring container metadata...'
      })
  ...
  async create(config) {
    // For backward compatibility, use async creation
