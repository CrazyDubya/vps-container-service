...
const DEFAULT_BACKEND_SERVER = 'https://31.97.128.225:3443'; // Use HTTPS for Cloudflare Workers
...
<body>
    ...
    <div class="container hidden" id="dashboardSection">
        ...
        <div class="info-box">
            ...
            <pre class="code">
# Using API Key
curl -H "x-api-key: YOUR_API_KEY" https://vps-containers.your-worker.workers.dev/api/containers

# Create a Python container
curl -X POST -H "x-api-key: YOUR_API_KEY" \\
     -H "Content-Type: application/json" \\
     -d '{"template": "python"}' \\
     https://vps-containers.your-worker.workers.dev/api/containers/create
            </pre>
        </div>
    ...
    <script>
        // All API calls go through the worker proxy
        const API_URL = '';  // Relative URLs - worker will proxy to backend
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const statusCheckInterval = setInterval(async () => {
                        try {
                            const statusResponse = await fetch(\`/api/containers/\${containerId}/status\`, {
                                headers: {'Authorization': \`Bearer \${authToken}\`}
                            });
